...
impl AppState {
    ...
    pub fn render_scope_graph(&mut self, source: &str) -> String {
        ...
        let mut hir = hir::lower_root(root);
        let graph = build_scope_graph(&hir, &syntax);
        resolve_hir(&mut hir, &graph);
        let type_result = check_hir(&hir);
        let snapshot = Snapshot::from_semantics(&hir, &graph, &type_result);
        let delta = Delta::new(self.previous.as_ref(), &snapshot);
        let html =
            render::render_semantics(&hir, &graph, &type_result, &delta, &reparse, &parse.errors);
