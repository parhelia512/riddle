...
impl TypeChecker<'_> {
    ...
    fn current_trait_assumptions(&mut self, ctx: &BodyCtx<'_>) -> Vec<TraitBound> {
        let bounds = self.current_generic_bounds(ctx);
    ...
    fn find_trait_impl_method_with_output(
        &mut self,
        receiver_ty: &Type,
        trait_arg_ty: Option<&Type>,
        method_arg_ty: Option<&Type>,
    ...
    fn pattern_uses_match_ergonomics(&self, pattern: &Pattern, expected: &Type) -> bool {
        match pattern {
            Pattern::Literal(_)
            | Pattern::Path { .. }
            | Pattern::Tuple { .. }
    ...
    fn enum_variant_is_unit_through_refs(&self, expected: &Type, name: &str) -> Option<bool> {
